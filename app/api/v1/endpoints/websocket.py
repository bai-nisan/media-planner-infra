...
async def websocket_endpoint(
    ...
    try:
        ...
        logger.info(f"WebSocket connection established for client {client_id}, session {session_id}")
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.error(f"Error in public WebSocket loop for client {client_id}: {e}")
                break
...
async def process_client_message(
    ...
    if message_type == "ping":
        ...
        await connection_manager.send_personal_message(
            '{"type": "pong", "timestamp": "' + str(uuid.uuid4()) + '"}',
            client_id
        )
        ...
        status_data = {
            ...
            "user_connections": connection_manager.get_user_connection_count(auth_info["user_id"]),
            "session_info": connection_manager.get_client_info(client_id)
        }
        await connection_manager.send_personal_message(
            str(status_data), client_id
        )
