...
from app.dependencies import get_tenant_id, get_user_id, get_temporal_service
...
class AgentTaskRequest(BaseModel):
    ...
    agent_type: str = Field(..., description="Target agent type (workspace, planning, insights, supervisor)")
    data: Dict[str, Any] = Field(default_factory=dict, description="Task data and parameters")
    context: Optional[Dict[str, Any]] = Field(default=None, description="Additional context for the task")
    async_execution: bool = Field(default=False, description="Whether to execute task asynchronously via Temporal")
...
class WorkflowRequest(BaseModel):
    ...
    workflow_type: str = Field(default="campaign_planning", description="Type of workflow to execute")
    ...
    tenant_specific: bool = Field(default=True, description="Whether this is tenant-specific")
...
class WorkflowResponse(BaseModel):
    ...
    results: Optional[Dict[str, Any]] = Field(default=None, description="Workflow results")
    completion_score: Optional[float] = Field(default=None, description="Workflow completion score (0-1)")
...
class AgentHealthResponse(BaseModel):
    ...
    agents: Dict[str, Dict[str, Any]] = Field(..., description="Individual agent health status")
...
async def execute_agent_task(
    ...
    try:
        ...
        task_id = f"task_{agent_type.value}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S_%f')}"
