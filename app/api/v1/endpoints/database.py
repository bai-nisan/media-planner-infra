...
async def get_campaign_context(
    ...
    tenant_id: Optional[str] = Query(
        None, description="Tenant ID for multi-tenant access"
    ),
    db_bridge: DatabaseBridge = Depends(get_database_bridge),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving campaign context: {str(e)}"
        )


@router.get("/tenant-context/{tenant_id}", response_model=Optional[Dict[str, Any]])
async def get_tenant_context(
    tenant_id: str, db_bridge: DatabaseBridge = Depends(get_database_bridge)
) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving tenant context: {str(e)}"
        )
