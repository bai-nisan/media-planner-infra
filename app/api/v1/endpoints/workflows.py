...
class WorkflowRequest(BaseModel):
    ...
    folder_id: Optional[str] = Field(
        None, description="Specific Google Drive folder ID to search"
    )
    search_keywords: Optional[List[str]] = Field(
        None, description="Keywords to search for in file names"
    )
    update_sheets: bool = Field(
        False, description="Whether to write results back to sheets"
    )
...
async def sync_data_async(
    ...
    try:
        ...
        background_tasks.add_task(
            _execute_sync_workflow, workflow_service, context, request
        )
