...
class TemporalClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            logger.info(
                f"Connecting to Temporal server at {self.settings.temporal_address}"
            )
            ...
            logger.info(
                f"Successfully connected to Temporal namespace: {self.settings.TEMPORAL_NAMESPACE}"
            )
    ...
    def _get_data_converter(self) -> Optional[DataConverter]:
        ...
        if self.settings.TEMPORAL_DATA_CONVERTER == "json":
            ...
            return DataConverter(payload_converter_class=JSONPlainPayloadConverter)
    ...
    async def execute_workflow(
        self, workflow_class, *args, workflow_id: str, task_queue: str, **kwargs
    ) -> Any:
        """
