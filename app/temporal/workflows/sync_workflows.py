...
from .google_ads_workflows import GoogleAdsIntegrationWorkflow, GoogleAdsCampaignSyncWorkflow
from .meta_ads_workflows import MetaAdsIntegrationWorkflow, MetaAdsCampaignSyncWorkflow  
from .google_drive_workflows import GoogleDriveIntegrationWorkflow, GoogleDriveFileSyncWorkflow
...
class ScheduledSyncWorkflow:
    ...
    async def run(self, sync_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        platforms = sync_config.get("platforms", ["google_ads", "meta_ads", "google_drive"])
        sync_interval = sync_config.get("sync_interval", 60)  # minutes
        full_sync_interval = sync_config.get("full_sync_interval", 24)  # hours
        
        workflow.logger.info(f"Starting scheduled sync for tenant {tenant_id} across platforms: {platforms}")
        ...
        try:
            ...
            if validation_result.get("conflicts"):
                conflict_resolution = await workflow.execute_activity(
                    resolve_data_conflicts,
                    {
                        ...
                        "resolution_strategy": sync_config.get("conflict_resolution", "latest_wins")
                    },
            ...
            await workflow.execute_activity(
                send_notification,
                {
                    ...
                    "recipients": sync_config.get("notification_settings", {}).get("success", [])
                },
                start_to_close_timeout=timedelta(minutes=3),
                retry_policy=RetryPolicy(maximum_attempts=2)
            )
            
            workflow.logger.info(f"Scheduled sync completed successfully for tenant {tenant_id}")
            return {
                "status": "success",
                "tenant_id": tenant_id,
                "sync_results": sync_results,
            ...
            workflow.logger.error(f"Scheduled sync failed for tenant {tenant_id}: {str(e)}")
            ...
            await workflow.execute_activity(
                send_notification,
                {
                    ...
                    "recipients": sync_config.get("notification_settings", {}).get("error", [])
                },
                start_to_close_timeout=timedelta(minutes=3),
                retry_policy=RetryPolicy(maximum_attempts=2)
            )
    ...
    def _should_do_full_sync(self, checkpoint: Dict[str, Any], full_sync_interval: int) -> bool:
    ...
    async def _sync_platform(
        ...
        workflow.logger.info(f"Starting {platform} sync (full={is_full_sync}) for tenant {tenant_id}")
...
class MultiTenantSyncOrchestratorWorkflow:
    ...
    async def run(self, orchestrator_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workflow.logger.info(f"Starting multi-tenant sync orchestration for {len(tenants)} tenants")
        ...
        workflow.logger.info(f"Multi-tenant sync orchestration completed for {len(tenants)} tenants")
        
        return {
            "status": "completed",
            "total_tenants": len(tenants),
            "successful_syncs": len([r for r in all_results.values() if r.get("status") == "success"]),
            "failed_syncs": len([r for r in all_results.values() if r.get("status") == "failed"]),
            "results": all_results,
            "completed_at": datetime.utcnow().isoformat()
        }
    
    def _prioritize_tenants(self, tenants: List[Dict], config: Dict) -> List[List[Dict]]:
        ...
        for i in range(0, len(tenant_group), max_concurrent):
            ...
            for tenant_config in batch:
                task = workflow.execute_child_workflow(
                    ...
                    retry_policy=RetryPolicy(maximum_attempts=1)  # Handle retries at tenant level
                )
            ...
            for tenant_id, task in batch_tasks:
                try:
                    ...
                    workflow.logger.error(f"Sync failed for tenant {tenant_id}: {str(e)}")
                    results[tenant_id] = {
                        "status": "failed",
                        "tenant_id": tenant_id,
                        "error": str(e),
...
class ConflictResolutionWorkflow:
    ...
    async def run(self, conflict_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workflow.logger.info(f"Starting conflict resolution for tenant {tenant_id} with {len(conflicts)} conflicts")
        ...
        for conflict in conflicts:
            try:
                ...
                workflow.logger.error(f"Failed to resolve conflict {conflict.get('id', 'unknown')}: {str(e)}")
                escalated_conflicts.append(conflict)
        ...
        if escalated_conflicts:
            await workflow.execute_activity(
                send_notification,
                {
                    ...
                    "recipients": conflict_config.get("escalation_rules", {}).get("recipients", [])
                },
                start_to_close_timeout=timedelta(minutes=3),
                retry_policy=RetryPolicy(maximum_attempts=2)
            )
        
        workflow.logger.info(f"Conflict resolution completed: {len(resolved_conflicts)} resolved, {len(escalated_conflicts)} escalated")
...
class SyncHealthMonitorWorkflow:
    ...
    async def run(self, monitor_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workflow.logger.info(f"Starting sync health monitoring with {monitoring_interval}min interval")
                ...
                if health_check_result.get("alerts_needed"):
                    await workflow.execute_activity(
                        send_notification,
                        {
                            ...
                            "recipients": monitor_config.get("notification_settings", {}).get("health_alerts", [])
                        },
                        start_to_close_timeout=timedelta(minutes=3),
                        retry_policy=RetryPolicy(maximum_attempts=2)
                    )
