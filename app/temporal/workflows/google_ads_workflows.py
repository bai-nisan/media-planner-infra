...
class GoogleAdsIntegrationWorkflow:
    ...
    async def run(
        ...
        """
        integration_id = (
            f"google_ads_{account_id}_{workflow.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        try:
            ...
            workflow.logger.info(
                f"Starting Google Ads authentication for account {account_id}"
            )
...
class GoogleAdsCampaignSyncWorkflow:
    ...
    async def run(
        ...
        """
        sync_id = (
            f"campaign_sync_{account_id}_{workflow.now().strftime('%Y%m%d_%H%M%S')}"
        )
