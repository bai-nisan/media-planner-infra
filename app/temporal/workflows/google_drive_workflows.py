...
class GoogleDriveIntegrationWorkflow:
    ...
    @workflow.run
    async def run(
        self, credentials: Dict[str, Any], tenant_id: str, sync_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute the complete Google Drive integration process."""
        integration_id = (
            f"google_drive_{tenant_id}_{workflow.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        try:
            ...
            for file_info in files:
                try:
                    ...
                    parsed_files.append(
                        {"file_info": file_info, "parsed_content": parsed_content}
                    )

                except Exception as e:
                    workflow.logger.warning(
                        f"Failed to process file {file_info['file_id']}: {str(e)}"
                    )

            # Step 4: Transform data
            raw_data = {"files": parsed_files, "tenant_id": tenant_id}
            ...
            return {
                ...
                "status": (
                    "success" if validation_result["is_valid"] else "validation_failed"
                ),
                "tenant_id": tenant_id,
                "data_summary": {
                    ...
                    "storage_status": (
                        storage_result["success"] if storage_result else False
                    ),
                },
...
class GoogleDriveFileSyncWorkflow:
    ...
    async def run(
        ...
        sync_id = (
            f"drive_file_sync_{tenant_id}_{workflow.now().strftime('%Y%m%d_%H%M%S')}"
        )
