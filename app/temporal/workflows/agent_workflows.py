...
class AgentTaskWorkflow:
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workflow.logger.info(
            f"Starting agent task workflow: {workflow.info().workflow_id}"
        )
        ...
        try:
            ...
            workflow.logger.info(
                f"Agent task completed successfully: {workflow.info().workflow_id}"
            )
...
class MultiAgentWorkflow:
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        workflow.logger.info(
            f"Starting multi-agent workflow: {workflow.info().workflow_id}"
        )
        ...
        try:
            ...
            if not tenant_id:
                raise ApplicationError(
                    "tenant_id is required for multi-agent workflows"
                )
            ...
            workflow.logger.info(
                f"Multi-agent workflow completed: {workflow.info().workflow_id}"
            )
...
class AgentHealthMonitorWorkflow:
    ...
    async def run(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            maintenance_actions = []
            if health_results.get("healthy_agents", 0) < health_results.get(
                "total_agents", 0
            ):
            ...
            if maintenance_actions:
                workflow.logger.info(
                    f"Scheduling maintenance actions: {maintenance_actions}"
                )
