...
async def authenticate_google_ads(
    ...
    try:
        activity.logger.info(
            f"Authenticating Google Ads account {account_id} for tenant {tenant_id}"
        )
        ...
        activity.logger.info(
            f"Successfully authenticated Google Ads account {account_id}"
        )
        return auth_result
...
async def fetch_google_ads_reports(
    ...
    try:
        account_id = auth_data["account_id"]
        activity.logger.info(
            f"Generating {report_type} report for account {account_id}"
        )
        ...
        if "campaigns" in raw_data:
            for campaign in raw_data["campaigns"]:
                transformed_campaign = {
                    ...
                    "cpc": (
                        (campaign["cost_micros"] / 1000000) / campaign["clicks"]
                ...
                transformed_data["summary"]["total_cost"] += (
                    campaign["cost_micros"] / 1000000
                )
                transformed_data["summary"]["total_conversions"] += campaign[
                    "conversions"
                ]
                transformed_data["summary"]["total_conversion_value"] += campaign[
                    "conversion_value"
                ]
