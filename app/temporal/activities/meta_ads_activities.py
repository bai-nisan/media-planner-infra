...
async def authenticate_meta_ads(
    ...
    try:
        activity.logger.info(f"Authenticating Meta Ads account {account_id} for tenant {tenant_id}")
        ...
        activity.logger.info(f"Successfully authenticated Meta Ads account {account_id}")
        return auth_result
...
async def fetch_meta_ads_insights(
    ...
    try:
        account_id = auth_data["account_id"]
        activity.logger.info(f"Fetching {level} insights for Meta Ads account {account_id}")
...
@activity.defn
async def fetch_meta_ads_audiences(
    auth_data: Dict[str, Any]
) -> List[Dict[str, Any]]:
