...
async def authenticate_google_drive(
    credentials: Dict[str, Any], tenant_id: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        activity.logger.info(
            f"Successfully authenticated Google Drive for tenant {tenant_id}"
        )
        return auth_result
...
async def fetch_google_drive_files(
    ...
    try:
        ...
        activity.logger.info(
            f"Successfully fetched {len(files)} files from Google Drive"
        )
        return files
...
async def download_google_drive_file(
    auth_data: Dict[str, Any], file_id: str, export_format: Optional[str] = None
) -> Dict[str, Any]:
...
async def parse_google_drive_content(
    file_content: Dict[str, Any], parsing_config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        parsed_content = {
            ...
            "metadata": {
                ...
                "has_formulas": True,
                "has_charts": False,
                "last_modified": file_content.get(
                    "modified_time", datetime.utcnow().isoformat()
                ),
            },
            "quality": {"completeness": 0.95, "consistency": 0.90, "accuracy": 0.92},
        }
...
async def transform_google_drive_data(
    parsed_content: Dict[str, Any], transformation_config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
                ...
                if "campaign" in sheet["sheet_name"].lower():
                    ...
                    for i, row in enumerate(sheet.get("data_preview", [])):
                        if len(row) >= 4:
                            ...
                            transformed_data["budget_allocations"].append(
                                budget_allocation
                            )
        ...
        activity.logger.info(
            f"Successfully transformed Google Drive data from file {file_id}"
        )
