...
from ..schedulers import SyncScheduler, setup_default_sync_schedules, SYNC_SCHEDULE_PRESETS
...
async def main():
    ...
    success = await scheduler.pause_tenant_sync_schedule(
        "startup-inc", 
        "Temporary pause for system maintenance"
    )
    ...
    for preset_name, preset_config in SYNC_SCHEDULE_PRESETS.items():
        print(f"   • {preset_name}: {preset_config['cron']} - {preset_config['description']}")
