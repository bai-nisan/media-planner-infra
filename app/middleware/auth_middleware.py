...
from app.services.auth import TokenData, get_auth_service
...
class JWTAuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, exclude_paths: Optional[list] = None):
        ...
        logger.info(
            f"JWT Auth Middleware initialized with {len(self.exclude_paths)} excluded paths"
        )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if authorization:
            ...
            if token:
                try:
                    ...
                    # Invalid token
                    logger.warning(
                        f"Invalid token for path {request.url.path}: {e.detail}"
                    )
                    return JSONResponse(
        ...
        if not token_data:
            logger.warning(
                f"No valid token provided for protected path: {request.url.path}"
            )
            return JSONResponse(
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if authorization:
            ...
            try:
                ...
                logger.debug(
                    f"Optional auth: invalid token provided for {request.url.path}"
                )
            except Exception as e:
