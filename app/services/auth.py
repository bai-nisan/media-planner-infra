...
class AuthenticationService:
    ...
    def __init__(self):
        ...
        if hasattr(self.settings, "SUPABASE_URL") and hasattr(
    ...
    def create_access_token(
        ...
        try:
            ...
            logger.info(
                f"Created access token for subject: {data.get('sub', 'unknown')}"
            )
            return encoded_jwt
        ...
        try:
            ...
            token_data = TokenData(
                ...
                scopes=[
                    ...
                    "external:read",
                    "external:write",
                ],  # Default scopes for Supabase users
                service_type="user",
    ...
    def create_service_token(
        ...
        return self.create_access_token(
            data=token_data, expires_delta=expires_delta, scopes=service.permissions
        )
    ...
    def verify_service_identity(self, token_data: TokenData) -> ServiceIdentity:
        ...
        if service_id not in self.service_registry:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Unknown service identity"
            )
