...
class AuthServiceClient:
    ...
    async def get_service_token(
        ...
        try:
            ...
            logger.error(
                f"Failed to get service token for {service_id}: {e.response.text}"
            )
            raise AuthServiceError(
                f"Service token generation failed: {e.response.text}"
            )
    ...
    async def store_google_credentials(
        self, service_id: str, credentials: Credentials, tenant_id: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            response = await self.client.post(
                ...
                json={"tenant_id": tenant_id, "credentials": creds_data},
            ...
            logger.error(
                f"Failed to store credentials for {service_id}: {e.response.text}"
            )
    ...
    async def retrieve_google_credentials(
        ...
        try:
            ...
            if e.response.status_code != 404:
                logger.error(
                    f"Failed to retrieve credentials for {service_id}: {e.response.text}"
                )
                raise AuthServiceError(
                    f"Credential retrieval failed: {e.response.text}"
                )
            return None
        except Exception as e:
    ...
    async def validate_credentials(
        self, service_id: str, tenant_id: Optional[str] = None
        ...
        try:
            credentials = await self.retrieve_google_credentials(
                service_id, tenant_id, auto_refresh=False
            )
    ...
    async def revoke_credentials(
        ...
        try:
            ...
            logger.error(
                f"Failed to revoke credentials for {service_id}: {e.response.text}"
            )
...
async def get_agent_credentials(
    agent_type: str, tenant_id: Optional[str] = None
) -> Optional[Credentials]:
    """
