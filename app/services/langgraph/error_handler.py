...
class ErrorHandler:
    ...
    async def handle_error(
        ...
        if (
            severity in [ErrorSeverity.CRITICAL, ErrorSeverity.FATAL]
            and self.enable_alerting
        ):
    ...
    async def retry_with_backoff(
        self, func: Callable, config: Optional[RetryConfig] = None, *args, **kwargs
    ):
    ...
    def register_resolution_strategy(
        self, category: ErrorCategory, strategy: Callable[[ErrorRecord], bool]
    ):
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        recent_errors = [
            {
                ...
                "message": (
                    err.message[:100] + "..." if len(err.message) > 100 else err.message
                ),
            }
        ...
        return {
            ...
            "resolution_attempts": sum(
                1 for err in self.error_records if err.resolution_attempted
            ),
        ...
        if strategy:
            try:
                resolved = (
                    ...
                    else strategy(error_record)
                )
                error_record.resolved = resolved

                if resolved:
                    logger.info(
                        f"Error {error_record.error_id} resolved using strategy for {error_record.category.value}"
                    )
