...
class RateLimitConfig:
    ...
    operation_weights: Dict[StateOperationType, float] = field(default_factory=lambda: {
...
class StateManager:
    ...
    def __init__(
        ...
        self._operation_semaphore = asyncio.Semaphore(self.resource_limits.max_concurrent_operations)
                ...
                try:
                    ...
                    if self.supabase_client:
                        state_record = {
                            ...
                            "checksum": await self._calculate_checksum(serialized_state)
                        }
    ...
    async def load_state(
        ...
        try:
            ...
            async with self._operation_semaphore:
                ...
                try:
                    if self.supabase_client:
                        query = self.supabase_client.table("agent_states")\
                            .select("state_data")\
    ...
    async def cleanup_tenant_state(
        ...
        try:
            ...
            if self.supabase_client:
                ...
                logger.info(f"Cleaned up {deleted_count} old states for tenant: {tenant_id}")
    ...
    async def get_tenant_metrics(self, tenant_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_entries": len([k for k in self._state_cache.keys() if k.startswith(f"{tenant_id}:")]),
    ...
    async def get_system_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recovery_checkpoints": sum(len(cp) for cp in self._recovery_checkpoints.values()),
            "rate_limit_config": {
    ...
    async def _check_rate_limits(self, tenant_id: str, operation_type: StateOperationType) -> bool:
        ...
        while request_history and now - request_history[0] > 60:  # Remove requests older than 1 minute
            request_history.popleft()
        
        # Count recent requests
        recent_requests = sum(1 for req_time in request_history if now - req_time <= 60)
        very_recent_requests = sum(1 for req_time in request_history if now - req_time <= 1)
        ...
        per_second_ok = weighted_very_recent < self.rate_limit_config.max_requests_per_second
    ...
    async def _check_resource_limits(self, state_data: AgentState, operation_id: str) -> bool:
        """Check if operation is within resource limits."""
        # Check concurrent operations
        if len(self._active_operations) >= self.resource_limits.max_concurrent_operations:
        ...
        if current_memory_mb > self.resource_limits.max_memory_usage_mb:
            self._metrics["resource_limit_violations"] += 1
            logger.warning(f"Memory limit exceeded: {current_memory_mb}MB > {self.resource_limits.max_memory_usage_mb}MB")
            return False
        ...
        try:
            ...
            if state_size_mb > self.resource_limits.max_state_size_mb:
                self._metrics["resource_limit_violations"] += 1
                logger.warning(f"State size limit exceeded: {state_size_mb}MB > {self.resource_limits.max_state_size_mb}MB")
                return False
        except Exception as e:
    ...
    async def _create_checkpoint(self, state_id: str, state_data: AgentState, tenant_id: Optional[str]):
    ...
    async def _handle_rate_limit_violation(self, tenant_id: Optional[str], operation_id: str):
    ...
    async def _handle_operation_error(self, error: Exception, operation_id: str, tenant_id: Optional[str]):
