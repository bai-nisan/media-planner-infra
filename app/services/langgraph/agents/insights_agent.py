...
from ..tools.insights_tools import (
    DataAnalyzer,
    InsightGenerator,
...
class InsightsAgent(BaseAgent):
    ...
    async def _handle_performance_analysis(
        ...
        analysis_result = await self.data_analyzer.execute(
            performance_data=performance_data, analysis_type=analysis_type
        )
        ...
        return {
            ...
            "key_findings": await self._extract_key_findings(
                analysis_result, performance_insights
            ),
    ...
    async def _handle_insight_generation(
        ...
        insights_result = await self.insight_generator.execute(
            analysis_results=analysis_results, campaign_context=campaign_context
        )
        ...
        return {
            ...
            "execution_status": "completed",
            "confidence_metrics": await self._calculate_insight_confidence(
                insights_result
            ),
            "impact_assessment": await self._assess_insight_impact(insights_result),
        }

    async def _handle_benchmark_comparison(
        self, task_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        evaluation_result = await self.performance_evaluator.execute(
            campaign_data=campaign_data, benchmarks=custom_benchmarks
        )
    ...
    async def _handle_comprehensive_analysis(
        ...
        performance_analysis = await self._handle_performance_analysis(
            {"performance_data": performance_data}
        )
        trend_analysis = await self._handle_trend_analysis(
            {"time_series_data": time_series_data}
        )
        benchmark_analysis = await self._handle_benchmark_comparison(
        ...
        comprehensive_insights = await self.insight_generator.execute(
            analysis_results=combined_analysis, campaign_context=campaign_context
        )
    ...
    async def _calculate_performance_scores(
        ...
        overall_score = (
            round(total_score / metric_count, 1) if metric_count > 0 else 5.0
        )
        ...
        return {
            ...
            "performance_grade": (
                "A"
                if overall_score >= 9
                else "B" if overall_score >= 7 else "C" if overall_score >= 5 else "D"
            ),
    ...
    async def _extract_key_findings(
        ...
        for metric, trend_data in trend_analysis.items():
            ...
            if direction == "increasing" and significance == "significant":
                ...
                findings.append(
                    f"📉 {metric.upper()} showing significant downward trend"
                )
            ...
            time_series.append(
                {
                    ...
                    "ctr": (
                        (base_clicks / base_impressions * 100)
                        if base_impressions > 0
                        else 0
                    ),
                    "conversion_rate": (
                        (base_conversions / base_clicks * 100) if base_clicks > 0 else 0
                    ),
    ...
    async def _analyze_trend_implications(
        self, trend_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for metric, trend_info in metric_trends.items():
            ...
            if trend_direction == "increasing":
                if metric in ["impressions", "clicks", "conversions"]:
                    implications["business_impact"].append(
                        f"Growing {metric} indicates positive campaign momentum"
                    )
                    implications["strategic_considerations"].append(
                        f"Consider scaling {metric} growth strategies"
                    )
                elif metric == "spend":
                    implications["business_impact"].append(
                        f"Increasing spend requires ROI monitoring"
                    )
                    implications["immediate_actions"].append(
                        f"Review spend efficiency for {metric}"
                    )

            elif trend_direction == "decreasing":
                if metric in ["impressions", "clicks", "conversions"]:
                    implications["business_impact"].append(
                        f"Declining {metric} needs immediate attention"
                    )
                    implications["immediate_actions"].append(
                        f"Investigate and address {metric} decline"
                    )
                elif metric in ["cpc", "cpa"]:
                    implications["business_impact"].append(
                        f"Decreasing {metric} indicates improving efficiency"
                    )
                    implications["strategic_considerations"].append(
                        f"Maintain strategies driving {metric} improvements"
                    )

            if volatility > 0.3:
                implications["long_term_planning"].append(
                    f"High {metric} volatility requires stability measures"
                )
    ...
    async def _identify_significant_trends(
        self, trend_result: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for metric, trend_info in metric_trends.items():
            ...
            if volatility > 0.2 or trend_direction != "stable":
                significance_score = (
                    volatility * 10 if trend_direction == "stable" else 8
                )
                ...
                significant_trends.append(
                    {
                        ...
                        "interpretation": self._interpret_trend_significance(
                            metric, trend_direction, volatility
                        ),
                    }
                )
        ...
        return sorted(
            significant_trends, key=lambda x: x["significance_score"], reverse=True
        )[:5]

    def _interpret_trend_significance(
        self, metric: str, direction: str, volatility: float
    ) -> str:
    ...
    async def _generate_trend_forecast(
        ...
        for metric, trend_info in metric_trends.items():
            ...
            if direction == "increasing":
                forecast["predicted_trends"].append(
                    f"{metric} expected to continue upward trajectory"
                )
                if metric in ["impressions", "clicks", "conversions"]:
                    forecast["opportunities"].append(
                        f"Scale investment in {metric} growth"
                    )
                elif metric == "spend":
                    forecast["risk_factors"].append(
                        f"Monitor {metric} growth for budget impact"
                    )

            elif direction == "decreasing":
                forecast["predicted_trends"].append(
                    f"{metric} may continue declining without intervention"
                )
                if metric in ["impressions", "clicks", "conversions"]:
                    forecast["risk_factors"].append(
                        f"Address {metric} decline before further deterioration"
                    )

        return forecast

    async def _prioritize_insights(
        self, insights_result: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        return sorted(all_insights, key=lambda x: x["priority_score"], reverse=True)[
            :10
        ]
    ...
    def _determine_urgency(self, insight: Dict[str, Any]) -> str:
        ...
        if (
            ...
            or insight.get("type") == "performance_issue"
        ):
            return "urgent"
    ...
    async def _create_insights_action_plan(
        self, insights_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        insights_data = insights_result.get("insights", {})
        optimization_recommendations = insights_data.get(
    ...
    async def _assess_insight_impact(
        self, insights_result: Dict[str, Any]
    ) -> Dict[str, Any]:
