...
from ..workflows.state_models import AgentTask, TaskStatus, WorkspaceData
...
providing seamless authentication across multi-tenant environments.""",
        ...
        logger.info(
            f"Initialized WorkspaceAgent with auth service integration for tenant: {tenant_id or 'default'}"
        )
    ...
    async def process_task(
        self, state: MessagesState, task: Dict[str, Any]
    ) -> Command[
        Literal["supervisor_agent", "planning_agent", "insights_agent", "__end__"]
    ]:
        ...
        try:
            ...
            logger.info(
                f"WorkspaceAgent processing task: {task_type} with auth service integration"
            )
            ...
            if hasattr(state, "workspace_data") and isinstance(
                state.workspace_data, WorkspaceData
            ):
                await self._update_workspace_data(
                    state.workspace_data, task_type, result
                )
            ...
            logger.info(
                f"WorkspaceAgent successfully completed {task_type}, routing to {next_agent}"
            )
    ...
    async def _ensure_valid_credentials(self) -> bool:
        ...
        try:
            async with ManagedGoogleAuth(
                self.auth_manager, auto_refresh=True
            ) as credentials:
                if credentials is None:
                    raise ValueError(
                        "No valid Google credentials available from auth service"
                    )
    ...
    async def _handle_extract_google_sheets(
        ...
        async with ManagedGoogleAuth(self.auth_manager) as credentials:
            ...
            extracted_data = await self.sheets_reader.extract_data(
                spreadsheet_id, sheet_range
            )

        # Enhanced validation with data quality scoring
        validation_result = await self.data_validator.validate_extracted_sheet_data(
            extracted_data
        )
        ...
        logger.info(
            f"Extracted {campaigns_found} campaigns with quality score {data_quality_score}"
        )
        return result

    async def _handle_check_auth_status(
        self, task_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Auth status check completed for tenant {self.tenant_id}: {can_authenticate}"
            )
            return result
    ...
    async def _handle_validate_data(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if validation_type == "campaign_data":
            validation_result = await self.data_validator.validate_campaign_data(
                data_to_validate
            )
            ...
            validation_result = await self.data_validator.validate_data_structure(
                data_to_validate
            )
        ...
        logger.info(
            f"Data validation completed with quality score: {result['quality_score']}"
        )
        return result
    ...
    async def _handle_transform_data(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transformation_type = task_data.get(
        ...
        result = {
            ...
            in ["campaign_standardization", "budget_normalization"],
            "metadata": {
    ...
    async def _handle_analyze_workspace(
        ...
        analysis = {
            "workspace_quality": self._calculate_workspace_quality(
                discovery_result, validation_result
            ),
            "file_count": len(discovery_result.get("discovered_files", [])),
            "spreadsheet_count": len(
                [
                    f
                    for f in discovery_result.get("discovered_files", [])
        ...
        result = {
            ...
            "metadata": {
                ...
                "workspace_health": (
                    "healthy"
                    if analysis["workspace_quality"] > 0.7
        ...
        logger.info(
            f"Workspace analysis completed: quality={analysis['workspace_quality']:.2f}"
        )
        return result
    ...
    async def _handle_discover_files(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if file_types and file_types != ["all"]:
            discovered_files = [
                f for f in discovered_files if f.get("file_type") in file_types
            ]
        ...
        result = {
            ...
            "metadata": {
                ...
                "search_criteria": {"file_types": file_types, "keywords": keywords},
            },
        }
    ...
    async def _update_workspace_data(
        self, workspace_data: WorkspaceData, task_type: str, result: Dict[str, Any]
    ):
        ...
        try:
            if task_type == "extract_google_sheets":
                ...
                workspace_data.campaign_data = {
                    "campaigns": result.get("extracted_data", {}).get(
                        "parsed_campaigns", []
                    ),
                ...
                workspace_data.drive_files = result.get("discovery", {}).get(
                    "discovered_files", []
                )
    ...
    async def _standardize_campaign_data(
        ...
        for campaign in campaigns:
            standardized_campaign = {
                ...
                "budget": (
                    float(campaign.get("budget", 0)) if campaign.get("budget") else 0.0
                ),
    ...
    def _calculate_workspace_quality(
        self, discovery_result: Dict[str, Any], validation_result: Dict[str, Any]
    ) -> float:
    ...
    def _generate_workspace_recommendations(
        ...
        if file_count == 0:
            recommendations.append(
                "No campaign files found. Consider organizing campaign data in Google Drive."
            )
        elif file_count < 3:
            recommendations.append(
                "Limited campaign files found. Consider consolidating campaign data."
            )

        validation_score = validation_result.get("overall_score", 0.0)
        if validation_score < 0.5:
            recommendations.append(
                "Data quality is low. Review and clean up campaign data structure."
            )
        elif validation_score < 0.8:
            recommendations.append(
                "Data quality could be improved. Consider standardizing field names and formats."
            )

        errors = validation_result.get("errors", [])
        if errors:
            recommendations.append(
                f"Found {len(errors)} data validation errors that need attention."
            )

        if not recommendations:
            recommendations.append(
                "Workspace appears well-organized and data quality is good."
            )
    ...
    async def _manage_workspace(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await self.workspace_manager.execute_operation(
        ...
        return {
            "operation": operation,
            "result": result,
            "metadata": {"operation_time": datetime.utcnow().isoformat()},
        }
