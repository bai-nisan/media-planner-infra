...
class MonitoringService:
    ...
    def time_operation(
        ...
        try:
            ...
            self.record_metric(
                f"{operation_name}.duration", duration_ms, MetricType.TIMER, tags
            )

            # Update performance metrics
            self._update_performance_metrics(
                operation_name, duration_ms, not error_occurred
            )
    ...
    def end_trace(
        ...
        with self._traces_lock:
            if span_id in self._active_spans:
                ...
                logger.debug(
                    f"Ended trace: {trace_event.operation} ({trace_event.duration_ms:.2f}ms)"
                )
    ...
    def trace_operation(
        ...
        span_id = self.start_trace(
            operation, agent_role, workflow_stage, metadata=metadata
        )
        error_occurred = False
        error_message = None
        ...
        return {
            ...
            "alerts": {
                ...
                "recent_triggers": len(
                    [
                        a
                        for a in self._triggered_alerts
    ...
    def _update_performance_metrics(
        ...
        if operation not in self._performance_metrics:
            self._performance_metrics[operation] = PerformanceMetrics(
                operation=operation
            )
    ...
    def _get_recent_error_rate(self) -> float:
        ...
        total_operations = sum(
            m.total_calls for m in self._performance_metrics.values()
        )
    ...
    def _get_average_response_time(self) -> float:
        ...
        total_duration = sum(
            m.average_duration_ms for m in self._performance_metrics.values()
        )
