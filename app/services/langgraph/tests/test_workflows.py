...
class TestStateGraphTransitions:
    ...
    def test_route_from_workspace_failure(self, workflow, initial_state):
        ...
        initial_state.set_agent_result(
            AgentRole.WORKSPACE, {"validation_passed": False}
        )
    ...
    def test_route_from_supervisor_complete(self, workflow, initial_state):
        ...
        initial_state.set_agent_result(
            AgentRole.SUPERVISOR, {"next_action": "complete"}
        )
    ...
    def test_route_from_supervisor_retry(self, workflow, initial_state):
        ...
        initial_state.set_agent_result(
            AgentRole.SUPERVISOR, {"next_action": "retry_workspace"}
        )
...
class TestCommandPatterns:
    ...
    async def test_task_assignment_command(self, state):
        ...
        assert (
            state.active_tasks[0].description == "Analyze campaign performance metrics"
        )
    ...
    async def test_workflow_control_command(self, state):
        ...
        # Test pause command
        pause_command = WorkflowControlCommand(control_action="pause")
        ...
        # Test resume command
        resume_command = WorkflowControlCommand(control_action="resume")
        ...
        complete_command = WorkflowControlCommand(control_action="complete")
...
class TestStateModel:
    ...
    def test_task_management(self, state):
        ...
        # Add a task
        task = AgentTask(
            id="test-task-1", agent_role=AgentRole.WORKSPACE, description="Test task"
        )
        ...
        task2 = AgentTask(
            id="test-task-2", agent_role=AgentRole.PLANNING, description="Test task 2"
        )
