...
class SupervisorWorkflow:
    ...
    async def _insights_node(self, state: CampaignPlanningState) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update workflow stage
            state.transition_to_stage(
                WorkflowStage.INSIGHTS_GENERATION, AgentRole.INSIGHTS
            )
    ...
    async def _supervisor_node(self, state: CampaignPlanningState) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update workflow stage
            state.transition_to_stage(
                WorkflowStage.SUPERVISOR_REVIEW, AgentRole.SUPERVISOR
            )
    ...
    async def _completion_node(self, state: CampaignPlanningState) -> Dict[str, Any]:
        ...
        try:
            ...
            final_results = {
                ...
                "final_outputs": {
                    "workspace_data": (
                        state.workspace_data.dict() if state.workspace_data else {}
                    ),
                    "campaign_plan": (
                        state.campaign_plan.dict() if state.campaign_plan else {}
                    ),
                    "insights_data": (
                        state.insights_data.dict() if state.insights_data else {}
                    ),
                },
            }
    ...
    def _route_from_workspace(
        self, state: CampaignPlanningState
    ) -> Literal["planning", "supervisor", "error"]:
    ...
    def _route_from_planning(
        self, state: CampaignPlanningState
    ) -> Literal["insights", "workspace", "supervisor", "error"]:
    ...
    def _route_from_insights(
        self, state: CampaignPlanningState
    ) -> Literal["supervisor", "planning", "error"]:
    ...
    def _route_from_supervisor(
        self, state: CampaignPlanningState
    ) -> Literal["workspace", "planning", "insights", "complete", "error"]:
    ...
    async def execute_command(
        self, command: CommandInterface, state: CampaignPlanningState
    ) -> Dict[str, Any]:
    ...
    def get_workflow_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents_configured": len(
                [a for a in self.agents.values() if a is not None]
            ),
