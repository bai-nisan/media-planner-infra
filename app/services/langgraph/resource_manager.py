...
class ResourceManager:
    ...
    async def acquire_resource(
        ...
        try:
            ...
            await self.error_handler.handle_error(
                e, severity=ErrorSeverity.ERROR, category=ErrorCategory.RESOURCE_LIMIT
            )
            raise
        finally:
            if acquired:
    ...
    async def create_connection_pool(
        self, pool_name: str, max_size: int, connection_factory: Callable
    ) -> ConnectionPool:
    ...
    async def get_connection(self, pool_name: str, timeout: Optional[float] = None):
        ...
        try:
                ...
                if pool.available_connections > 0:
                    ...
                    if timeout:
                        await asyncio.wait_for(
                            self._wait_for_pool_connection(pool_name), timeout=timeout
                        )
                    else:
            ...
            await self.error_handler.handle_error(
                e, severity=ErrorSeverity.WARNING, category=ErrorCategory.RESOURCE_LIMIT
            )
            raise
        finally:
            if connection:
    ...
    def _initialize_semaphores(self):
        ...
        self._resource_locks = {
            ...
            ResourceType.DATABASE_CONNECTIONS: asyncio.Semaphore(
                self.limits.max_db_connections
            ),
            ResourceType.NETWORK_CONNECTIONS: asyncio.Semaphore(
                self.limits.max_network_connections
            ),
            ResourceType.AGENT_INSTANCES: asyncio.Semaphore(
                self.limits.max_agent_instances
            ),
            ResourceType.WORKFLOW_INSTANCES: asyncio.Semaphore(
                self.limits.max_workflow_instances
            ),
        }
    ...
    async def _monitor_resources(self):
        ...
        while self._is_monitoring:
            try:
                ...
                await self.error_handler.handle_error(
                    e, severity=ErrorSeverity.ERROR, category=ErrorCategory.SYSTEM
                )
    ...
    async def _update_resource_usage(self):
        ...
        try:
            ...
            self._current_usage = ResourceUsage(
                ...
                active_db_connections=len(
                    self._active_resources[ResourceType.DATABASE_CONNECTIONS]
                ),
                active_network_connections=len(
                    self._active_resources[ResourceType.NETWORK_CONNECTIONS]
                ),
                active_agent_instances=len(
                    self._active_resources[ResourceType.AGENT_INSTANCES]
                ),
                active_workflow_instances=len(
                    self._active_resources[ResourceType.WORKFLOW_INSTANCES]
                ),
    ...
    async def _check_resource_violations(self):
        ...
        if usage.memory_percent > self.limits.max_memory_percent:
            await self.error_handler.handle_error(
                Exception(
                    f"Memory usage exceeded limit: {usage.memory_percent}% > {self.limits.max_memory_percent}%"
                ),
                severity=ErrorSeverity.WARNING,
                category=ErrorCategory.RESOURCE_LIMIT,
            )
        ...
        if usage.cpu_percent > self.limits.max_cpu_percent:
            await self.error_handler.handle_error(
                Exception(
                    f"CPU usage exceeded limit: {usage.cpu_percent}% > {self.limits.max_cpu_percent}%"
                ),
                severity=ErrorSeverity.WARNING,
                category=ErrorCategory.RESOURCE_LIMIT,
            )
    ...
    async def _cleanup_resources(self):
        ...
        while self._is_monitoring:
            try:
                ...
                expired_timeouts = [
                    op_id
                    for op_id, task in self._active_timeouts.items()
                    if task.done()
                ]
                ...
                await self.error_handler.handle_error(
                    e, severity=ErrorSeverity.WARNING, category=ErrorCategory.SYSTEM
                )
    ...
    async def _cleanup_connection_pool(self, pool_name: str):
        ...
        async with self._pool_locks[pool_name]:
            ...
            if idle_connections > pool.max_size // 2:  # Keep half as minimum
                ...
                logger.debug(
                    f"Cleaned up {connections_to_close} idle connections from pool {pool_name}"
                )
    ...
    async def _wait_for_resource(
        self, resource_type: ResourceType, timeout: Optional[float]
    ):
