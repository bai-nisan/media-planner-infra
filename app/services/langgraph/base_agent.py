...
from .error_handler import ErrorCategory, ErrorHandler, ErrorSeverity, RetryConfig
from .monitoring import MetricType, MonitoringService
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.llm_circuit_breaker = self.error_handler.create_circuit_breaker(
            name=f"llm_{self.agent_id}", config=None  # Use defaults
        )
    ...
    def _get_agent_role(self) -> AgentRole:
        ...
        if isinstance(self.config.type, str):
            ...
            try:
                ...
                logger.warning(
                    f"Unknown agent type: {self.config.type}, defaulting to SUPERVISOR"
                )
            ...
            with self.monitoring.trace_operation(
                ...
                try:
                    ...
                    self.monitoring.increment_counter(
                        f"{operation_name}.success", tags={"agent_id": self.agent_id}
                    )
        ...
        with self.monitoring.time_operation(operation_name):
            try:
                ...
                if success:
                    ...
                    self.monitoring.increment_counter(
                        f"{operation_name}.failure", tags={"agent_id": self.agent_id}
                    )
                ...
                await self.error_handler.handle_error(
                    ...
                    context={"agent_id": self.agent_id, "operation": operation_name},
                )
                return False
        ...
        with self.monitoring.time_operation(operation_name):
            try:
                agent_state = await self.state_manager.load_state(
                    state_id=self.agent_id, tenant_id=tenant_id
                )
                ...
                if agent_state:
                    ...
                    self.monitoring.increment_counter(
                        f"{operation_name}.not_found", tags={"agent_id": self.agent_id}
                    )
                    return None
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.error_handler.handle_error(
                ...
                context={"agent_id": self.agent_id, "operation": "health_check"},
            )
