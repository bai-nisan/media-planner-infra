...
class GoogleSheetsClient:
    ...
    def batch_write(
        ...
        try:
            ...
            logger.info(f"Updated {total_updated} total cells across {len(updates)} ranges")
            return result
    ...
    def _parse_campaign_row(self, headers: List[str], row: List[str]) -> CampaignData:
        ...
        row_data = {header.lower().strip(): value for header, value in zip(headers, row)}
        
        # Extract common campaign fields
        campaign_name = row_data.get('campaign_name', '') or row_data.get('name', '') or row_data.get('campaign', '')
        ...
        # Extract platform
        platform = row_data.get('platform', '') or row_data.get('channel', '') or row_data.get('media_type', '')
        
        # Extract targeting info (store as dict)
        targeting = {}
        for key, value in row_data.items():
            if any(target_key in key for target_key in ['target', 'audience', 'demographic']):
                if value.strip():
                    targeting[key] = value
        ...
        metrics = {}
        for key, value in row_data.items():
            if any(metric_key in key for metric_key in ['impression', 'click', 'conversion', 'ctr', 'cpc', 'cpm']):
                if value.strip():
                    try:
            ...
            for sheet_file in sheet_files:
                ...
                if any(keyword in name_lower for keyword in campaign_keywords):
                    try:
                        ...
                        logger.warning(f"Could not get info for sheet {sheet_file.name}: {e}")
                        continue
