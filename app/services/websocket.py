...
class ConnectionManager:
    ...
    async def broadcast_to_tenant(
        ...
        exclude_clients = exclude_clients or []
        tenant_clients = [
            client_id for client_id, session in self.client_sessions.items()
            if (session.get("session_data", {}).get("tenant_id") == tenant_id and
                client_id not in exclude_clients)
        ]
    ...
    async def cleanup_inactive_connections(self, timeout_minutes: int = 30):
        ...
        for client_id in inactive_clients:
            ...
            if websocket:
                try:
                    await websocket.close(code=status.WS_1000_NORMAL_CLOSURE, reason="Inactive")
                except:
...
async def handle_websocket_message(
    ...
    try:
        ...
        if client_id in connection_manager.client_sessions:
            connection_manager.client_sessions[client_id]["last_activity"] = datetime.utcnow()
