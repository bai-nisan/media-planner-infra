...
class TestAuthServiceClient:
    ...
    @pytest.mark.asyncio
    async def test_store_google_credentials(self, auth_client, mock_httpx_client, mock_credentials):
        ...
        # Mock store response
        store_response = Mock()
        store_response.json.return_value = {"message": "Credentials stored successfully"}
    ...
    @pytest.mark.asyncio
    async def test_retrieve_google_credentials_not_found(self, auth_client, mock_httpx_client):
        ...
        credentials = await auth_client.retrieve_google_credentials("nonexistent_service")
    ...
    async def test_validate_credentials(self, auth_client):
        ...
        auth_client.retrieve_google_credentials = AsyncMock(return_value=mock_credentials)
...
class TestAuthServiceIntegratedManager:
    ...
    @pytest.mark.asyncio
    async def test_get_valid_credentials_from_service(self, auth_manager, mock_credentials):
        """Test getting credentials from auth service."""
        # Mock auth service client
        auth_manager.auth_client = AsyncMock()
        auth_manager.auth_client.__aenter__ = AsyncMock(return_value=auth_manager.auth_client)
        auth_manager.auth_client.__aexit__ = AsyncMock(return_value=None)
        auth_manager.auth_client.retrieve_google_credentials = AsyncMock(return_value=mock_credentials)
    ...
    @pytest.mark.asyncio
    async def test_get_valid_credentials_fallback_to_local(self, auth_manager, mock_credentials):
        ...
        assert credentials == mock_credentials
        auth_manager._sync_credentials_to_service.assert_called_once_with(mock_credentials)
    ...
    async def test_exchange_code_for_credentials(self, auth_manager, mock_credentials):
        ...
        with patch.object(auth_manager.__class__.__bases__[0], 'exchange_code_for_credentials', return_value=mock_credentials):
            ...
            auth_manager._store_credentials_in_service.assert_called_once_with(mock_credentials)
    ...
    async def test_refresh_credentials(self, auth_manager, mock_credentials):
        ...
        expired_credentials.expired = True  # This will now work because we're using a Mock
        ...
        auth_manager._store_credentials_in_service.assert_called_once_with(expired_credentials)
    ...
    async def test_revoke_credentials(self, auth_manager):
        ...
        with patch.object(auth_manager.__class__.__bases__[0], 'revoke_credentials', return_value=True):
    ...
    async def test_get_credential_status(self, auth_manager, mock_credentials):
        ...
        auth_manager.auth_client.__aenter__ = AsyncMock(return_value=auth_manager.auth_client)
        auth_manager.auth_client.__aexit__ = AsyncMock(return_value=None)
        auth_manager.auth_client.validate_credentials = AsyncMock(return_value=service_status)
        
        # Mock local status
        with patch.object(auth_manager.__class__.__bases__[0], 'get_valid_credentials', return_value=mock_credentials), \
             patch.object(type(mock_credentials), 'expired', new_callable=lambda: property(lambda self: False)):
            mock_credentials.expiry = datetime.utcnow() + timedelta(hours=1)
...
class TestManagedGoogleAuth:
    ...
    async def test_context_manager_with_refresh(self, settings, mock_credentials):
        ...
        with patch.object(type(mock_credentials), 'expired', new_callable=lambda: property(lambda self: True)):
            auth_manager = Mock()
            auth_manager.get_valid_credentials = AsyncMock(return_value=mock_credentials)
            auth_manager.refresh_credentials = AsyncMock(return_value=mock_credentials)
            
            async with ManagedGoogleAuth(auth_manager, auto_refresh=True) as credentials:
                assert credentials == mock_credentials
    ...
    async def test_context_manager_error_handling(self, settings):
        ...
        auth_manager = Mock()
        auth_manager.get_valid_credentials = AsyncMock(side_effect=Exception("Test error"))
...
class TestHelperFunctions:
    ...
    async def test_get_agent_credentials(self):
        ...
        with patch('app.services.auth_client.AuthServiceClient') as mock_client_class:
            ...
            mock_client.retrieve_google_credentials = AsyncMock(return_value=mock_credentials)
    ...
    async def test_get_workspace_credentials(self, settings, mock_credentials):
        ...
        with patch('app.services.google.auth_enhanced.AgentAuthManagerFactory.create_workspace_agent_auth') as mock_factory:
            mock_auth_manager = Mock()
            mock_auth_manager.get_valid_credentials = AsyncMock(return_value=mock_credentials)
    ...
    async def test_get_planning_credentials(self, settings, mock_credentials):
        ...
        with patch('app.services.google.auth_enhanced.AgentAuthManagerFactory.create_planning_agent_auth') as mock_factory:
            mock_auth_manager = Mock()
            mock_auth_manager.get_valid_credentials = AsyncMock(return_value=mock_credentials)
            mock_factory.return_value = mock_auth_manager
...
class TestIntegrationScenarios:
    ...
    async def test_full_credential_lifecycle(self, settings, mock_credentials):
        ...
        store_result = await auth_manager._store_credentials_in_service(mock_credentials)
        ...
        auth_manager.auth_client.validate_credentials = AsyncMock(return_value=status_mock)
