...
class TestGoogleSheetsReader:
    ...
    def test_initialization_with_defaults(self):
        ...
        with (
            patch(
                "app.services.langgraph.tools.workspace_tools.get_settings"
            ) as mock_get_settings,
            patch(
                "app.services.langgraph.tools.workspace_tools.GoogleAuthManager"
            ) as mock_auth_class,
        ):
    ...
    async def test_extract_data_success(
        self, mock_auth_manager, mock_settings, sample_sheet_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_extract_data_authentication_error(
...
class TestDataValidator:
    ...
    def test_initialization(self):
        ...
        assert (
            "campaign_name"
            in validator.validation_rules["campaign_data"]["required_fields"]
        )
        assert (
            "Google Ads"
            in validator.validation_rules["campaign_data"]["valid_platforms"]
        )
    ...
    async def test_validate_extracted_sheet_data_no_data(self):
        ...
        no_data = {"extraction_metadata": {"status": "no_data_found"}}
...
class TestFileParser:
    ...
    def test_google_drive_id_detection(self, mock_auth_manager, mock_settings):
        ...
        assert (
            parser._is_google_drive_id("1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms")
            is True
        )
...
class TestWorkspaceManager:
    ...
    @pytest.mark.asyncio
    async def test_discover_campaign_files_success(
        self, mock_auth_manager, mock_settings
    ):
        ...
        with (
            patch.object(manager, "_sheets_reader") as mock_sheets_reader,
    ...
    @pytest.mark.asyncio
    async def test_discover_campaign_files_authentication_error(
        self, mock_auth_manager, mock_settings
    ):
...
class TestGoogleAPIIntegrationWorkflow:
    ...
    async def test_complete_workflow_simulation(
        ...
        with patch.object(sheets_reader, "_sheets_client") as mock_sheets_client:
            ...
            validation_result = await validator.validate_extracted_sheet_data(
                extracted_data
            )
