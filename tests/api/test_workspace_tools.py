...
class TestGoogleSheetsReader:
    ...
    def sheets_reader(self, mock_auth_manager, mock_settings):
        ...
        return GoogleSheetsReader(auth_manager=mock_auth_manager, settings=mock_settings)
    ...
    async def test_initialization(self, mock_auth_manager, mock_settings):
        ...
        reader = GoogleSheetsReader(auth_manager=mock_auth_manager, settings=mock_settings)
    ...
    @pytest.mark.asyncio
    async def test_extract_data_success(self, sheets_reader, sample_sheet_data, sample_campaign_data):
        ...
        mock_sheets_client.read_range.return_value = Mock(values=sample_sheet_data["values"])
    ...
    async def test_extract_data_api_error(self, sheets_reader):
        ...
        # Mock API error
        mock_sheets_client = Mock(spec=GoogleSheetsClient)
        mock_sheets_client.get_spreadsheet_info.side_effect = Exception("API Error: Sheet not found")
        mock_sheets_client.__enter__.return_value = mock_sheets_client
        mock_sheets_client.__exit__.return_value = None
...
class TestDataValidator:
    ...
    @pytest.mark.asyncio
    async def test_validate_extracted_sheet_data_success(self, data_validator, sample_sheet_data):
    ...
    async def test_validate_extracted_sheet_data_missing_headers(self, data_validator):
        ...
        assert any("missing required header" in error.lower() for error in result["errors"])
    ...
    async def test_data_quality_scoring(self, data_validator, sample_sheet_data):
        ...
        result_low = await data_validator.validate_extracted_sheet_data(low_quality_data)
...
class TestWorkspaceManager:
    ...
    async def test_discover_campaign_files(self, workspace_manager):
        ...
        mock_sheets_reader = Mock()
        mock_sheets_reader.discover_campaign_sheets = AsyncMock(return_value={
        ...
        mock_drive_client.search_files = AsyncMock(return_value=mock_files[1:])  # Only docs
        mock_drive_client.__enter__.return_value = mock_drive_client
        mock_drive_client.__exit__.return_value = None
    ...
    async def test_validate_workspace(self, workspace_manager):
        ...
        mock_data_validator.validate_data = AsyncMock(return_value={
            "is_valid": True,
    ...
    async def test_execute_operation_discovery(self, workspace_manager):
        ...
        workspace_manager.discover_campaign_files = AsyncMock(return_value={
            "discovered_files": [],
    ...
    async def test_execute_operation_validation(self, workspace_manager):
        ...
        workspace_manager.validate_workspace = AsyncMock(return_value={
            "is_valid": True,
            "overall_score": 0.9
        })
        
        result = await workspace_manager.execute_operation("validate_workspace", {
            "files": test_files
        })
...
class TestWorkspaceToolsIntegration:
    ...
    @pytest.mark.skipif(
        ...
        reason="Integration tests require Google API setup"
    )
    @pytest.mark.asyncio
