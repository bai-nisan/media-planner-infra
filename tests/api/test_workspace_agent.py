...
from app.services.langgraph.workflows.state_models import MessagesState, WorkspaceData
...
class TestWorkspaceAgent:
    ...
    def workspace_agent(self, mock_auth_manager, mock_settings, monkeypatch):
        ...
        with (
            patch(
                "app.services.langgraph.base_agent.StateManager"
            ) as mock_state_manager_class,
            patch(
                "app.services.langgraph.base_agent.ErrorHandler"
            ) as mock_error_handler_class,
            patch(
                "app.services.langgraph.base_agent.ResourceManager"
            ) as mock_resource_manager_class,
            patch(
                "app.services.langgraph.base_agent.MonitoringService"
    ...
    async def test_initialization_with_dependencies(
        self, mock_auth_manager, mock_settings
    ):
    ...
    async def test_initialization_without_dependencies(self):
        ...
        with (
            patch(
                "app.services.langgraph.agents.workspace_agent.get_settings"
            ) as mock_get_settings,
            patch(
                "app.services.langgraph.agents.workspace_agent.GoogleAuthManager"
            ) as mock_auth_class,
        ):
    ...
    @pytest.mark.asyncio
    async def test_extract_google_sheets_success(
        self, workspace_agent, sample_message_state, sample_extracted_data
    ):
        ...
        workspace_agent.data_validator.validate_extracted_sheet_data = AsyncMock(
            return_value={
                "is_valid": True,
        ...
        task = {
            ...
            "data": {"spreadsheet_id": "test_sheet_id_123", "range": "A1:Z1000"},
        }
        ...
        assert (
            "WorkspaceAgent completed: extract_google_sheets"
        ...
        workspace_agent.sheets_reader.extract_data.assert_called_once_with(
            "test_sheet_id_123", "A1:Z1000"
        )
        workspace_agent.data_validator.validate_extracted_sheet_data.assert_called_once()

    @pytest.mark.asyncio
    async def test_extract_google_sheets_authentication_error(
        self, workspace_agent, sample_message_state
    ):
        ...
        task = {
            "type": "extract_google_sheets",
            "data": {"spreadsheet_id": "test_sheet_id_123"},
        }
    ...
    @pytest.mark.asyncio
    async def test_extract_google_sheets_missing_spreadsheet_id(
        self, workspace_agent, sample_message_state
    ):
        """Test handling of missing required parameters."""
        task = {"type": "extract_google_sheets", "data": {}}  # Missing spreadsheet_id
    ...
    async def test_validate_data_success(self, workspace_agent, sample_message_state):
        ...
        workspace_agent.data_validator.validate_extracted_sheet_data.assert_called_once_with(
            {"test": "data"}
        )

    @pytest.mark.asyncio
    async def test_transform_data_campaign_standardization(
        self, workspace_agent, sample_message_state, sample_extracted_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_workspace_success(
        self, workspace_agent, sample_message_state
    ):
        ...
        workspace_agent.workspace_manager.discover_campaign_files = AsyncMock(
            return_value=discovery_result
        )
        workspace_agent.workspace_manager.validate_workspace = AsyncMock(
            return_value=validation_result
        )

        task = {"type": "analyze_workspace", "data": {}}
    ...
    @pytest.mark.asyncio
    async def test_discover_files_with_filtering(
        self, workspace_agent, sample_message_state
    ):
    ...
    async def test_unknown_task_type_error(self, workspace_agent, sample_message_state):
        ...
        task = {"type": "unknown_task_type", "data": {}}
    ...
    @pytest.mark.asyncio
    async def test_workspace_data_state_update(
        self, workspace_agent, sample_extracted_data
    ):
        """Test proper WorkspaceData state updates."""
        # Create state with WorkspaceData
        state = {"messages": [], "workspace_data": WorkspaceData()}
    ...
    async def test_legacy_method_compatibility(
        ...
        workspace_agent._handle_extract_google_sheets = AsyncMock(
            return_value={"test": "result"}
        )

        # Call legacy method
        result = await workspace_agent._extract_google_sheets(
            {"spreadsheet_id": "test_id"}
        )

        # Verify delegation to new handler
        workspace_agent._handle_extract_google_sheets.assert_called_once_with(
            {"spreadsheet_id": "test_id"}
        )
    ...
    def test_workspace_quality_calculation(self, workspace_agent):
        ...
        quality = workspace_agent._calculate_workspace_quality(
    ...
    def test_workspace_recommendations_generation(self, workspace_agent):
        ...
        recommendations = workspace_agent._generate_workspace_recommendations(
            discovery_result, validation_result
        )
        ...
        discovery_result = {
            "discovered_files": [{"file": "test"}, {"file": "test2"}, {"file": "test3"}]
        }
...
class TestWorkspaceAgentIntegration:
    ...
    async def test_error_recovery_and_logging(
        ...
        with patch(
            "app.services.langgraph.agents.workspace_agent.logger"
        ) as mock_logger:
            result = await workspace_agent.process_task(sample_message_state, task)
