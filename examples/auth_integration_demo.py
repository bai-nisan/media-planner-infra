...
class AuthDemo:
    ...
    async def authenticate(
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Authentication successful! Token expires in {token_data['expires_in']} seconds"
            )
    ...
    async def check_health(self):
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Auth service healthy: {health_data['service']} v{health_data['version']}"
            )
    ...
    async def store_google_credentials(
        self, service_id: str, tenant_id: str = "default"
    ):
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Credentials stored successfully for {result['service_id']} (tenant: {result['tenant_id']})"
            )
        else:
    ...
    async def check_credential_status(
        ...
        if response.status_code == 200:
            ...
            print(
                f"  Expires in: {status.get('expires_in_seconds', 'Unknown')} seconds"
            )
    ...
    async def test_service_registry(self):
        ...
        response = await self.session.get(
            f"{self.base_url}/api/v1/auth/services", headers=self.headers
        )
