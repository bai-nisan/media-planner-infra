...
def get_temporal_service(request: Request) -> TemporalService:
    ...
    """
    if (
        not hasattr(request.app.state, "temporal_service")
        or request.app.state.temporal_service is None
    ):
        raise HTTPException(status_code=503, detail="Temporal service is not available")
...
def get_temporal_client(request: Request):
    ...
    """
    if (
        not hasattr(request.app.state, "temporal_client")
        or request.app.state.temporal_client is None
    ):
...
async def verify_temporal_health(
    ...
    if health_status.get("status") != "healthy":
        raise HTTPException(
            status_code=503, detail=f"Temporal service is unhealthy: {health_status}"
        )
...
async def get_agent_service_dependency(request: Request) -> Optional["AgentService"]:
    ...
    if agent_service is None:
        raise HTTPException(
            status_code=503, detail="Agent service failed to initialize"
        )
